    ...
    for doc in docs_to_generate_queries:
        ...
        for query_text in query_response.get_queries():
            if len(data_store.get_queries()) >= config.num_queries_needed:
                return
            query_obj: Query = Query(text=query_text)

            # query_id could be different than query_obj.id if the text is already cached
            query_id: str = data_store.add_query(query_obj)
            data_store.create_rating_score(
                query_id, doc.id, max(config.relevance_label_set),

...
from src.model import Document, Query  # <-- usar los modelos pydantic unificados
...
# data structure
from src.data_store import DataStore  # <-- nuevo DataStore minimal/YAGNI
    ...
    if config.queries is not None:
        with open(config.queries, "r", encoding="utf-8") as file:
            for line in file:
                cleaned = line.strip()
                if cleaned:
                    data_store.add_query(Query(text=cleaned))
...
def generate_and_add_queries(config: Config, data_store: DataStore, llm_service: LLMService, search_engine: BaseSearchEngine) -> None:
    ...
    for doc in docs_to_generate_queries:
        ...
        for query_text in query_response.get_queries():
            ...
            qid: str = data_store.add_query(q)
            # assign max rating by default policy
            data_store.create_rating_score(
                qid, doc.id, max(config.relevance_label_set),
...
def add_cartesian_product_scores(config: Config, data_store: DataStore, llm_service: LLMService) -> None:
    ...
    for q in data_store.get_queries():
        for doc in data_store.get_documents():
            if not data_store.has_rating_score(q.id, doc.id):
                score_resp: LLMScoreResponse = llm_service.generate_score(
                    doc, q.text, config.relevance_scale, config.save_llm_explanation
                )
                data_store.create_rating_score(
                    q.id, doc.id, score_resp.get_score(),
                    score_resp.get_explanation() if getattr(config, "save_llm_explanation", False) else None
                )
...
def expand_docset_with_search_engine_topK(config: Config, data_store: DataStore,
    ...
    for q in data_store.get_queries():
        docs_eval: List[Document] = search_engine.fetch_for_evaluation(
            keyword=q.text, query_template=config.query_template, doc_fields=config.doc_fields
        )
...
if __name__ == "__main__":
    ...
    writer: AbstractWriter = WriterFactory.build(config, data_store)  # mantiene tu firma actual

    # load already existing queries
    ...
    # score the initial docset
    add_cartesian_product_scores(config, data_store, service)

    # expand the docset with search engine topK (including direct ratings)
    ...
    if getattr(config, "save_llm_explanation", False):

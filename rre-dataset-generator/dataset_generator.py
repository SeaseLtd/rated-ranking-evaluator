...
from src.writers.llm_explanation_writer import LlmExplanationWriter
...
if __name__ == "__main__":
    ...
    for doc in docs_to_generate_queries:
        ...
        llm_query_response = service.generate_queries(doc, num_queries_per_doc)
        queries = llm_query_response.get_queries()
        for query_text in queries:
            ...
            data_store.add_rating_score(query_id, doc.id, max(config.relevance_label_set),
                                        "Human Reasoning: max score is assigned when generated query")
    ...
    for query_rating_context in data_store.get_queries():
        for doc in data_store.get_documents():
            if not data_store.has_rating_score(query_rating_context.get_query_id(), doc.id):
                llm_score_response = service.generate_score(data_store.get_document(doc.id),
                                                            query_rating_context.get_query(),
                                                            config.relevance_scale,
                                                            config.save_llm_explanation)
                data_store.add_rating_score(query_rating_context.get_query_id(), doc.id,
                                            llm_score_response.score, llm_score_response.reasoning)

    if config.save_llm_explanation:
        llm_explanation_writer = LlmExplanationWriter(data_store)
        llm_explanation_writer.write(config.llm_explanation_destination)
        log.info(f"LLM explanation is saved into: {config.llm_explanation_destination}")

        ...
        for doc in data_store.get_documents():
            if not data_store.has_rating_score(query_rating_context.get_query_id(), doc.id):
                ...
                data_store.add_rating_score(query_rating_context.get_query_id(),
                                            doc.id,
                                            score_response.get_score(), score_response.get_reasoning())
...
if __name__ == "__main__":
    ...
    if config.save_llm_explanation:
        data_store.export_all_records_with_reasoning(config.llm_explanation_destination)
        log.info(f"Dataset with LLM reasoning is saved into: {config.llm_explanation_destination}")

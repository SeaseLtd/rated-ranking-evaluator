...
def retrieve_and_score_documents(config: Config, data_store: DataStore, llm_service: LLMService, search_engine: BaseSearchEngine) -> None:
    for query_rating_context in data_store.get_queries():
        ...
        for doc in docs_eval:
            if not data_store.has_rating_score(query_rating_context.get_query_id(), doc.id):
                score_response: LLMScoreResponse = llm_service.generate_score(doc,
                                                                              query_rating_context.get_query_text(),
                                                                              config.relevance_scale,
...
if __name__ == "__main__":
    ...
    retrieve_and_score_documents(config, data_store, service, search_engine)

...
def solr_config():
    ...
    return Config.load("tests/integration/resources/good_config.yaml")


def test_end_to_end_pipeline(solr_config):
    ...
    search_engine: BaseSearchEngine = SearchEngineFactory.build(search_engine_type=solr_config.search_engine_type,
                                                                endpoint=solr_config.search_engine_collection_endpoint)
    ...
    writer: AbstractWriter = WriterFactory.build(solr_config.output_format, data_store)

    # pipeline starts
    add_user_queries(solr_config, data_store)
    ...
    generate_and_add_queries(solr_config, data_store, service, docs_to_generate_queries)

    retrieve_and_add_documents(solr_config, data_store, search_engine)

    add_cartesian_product_scores(solr_config, data_store, service)

    writer.write(solr_config.output_destination)

    log.info(f"Synthetic Dataset has been generated in: {solr_config.output_destination}")

...
def main() -> None:
    ...
    evaluation = mteb.MTEB(tasks=[task_map.get(config.task_to_evaluate, CustomRetrievalTask())])
    evaluation.run(model, output_folder=config.output_dest, overwrite_results=True, config=config)

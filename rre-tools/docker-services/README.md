...
### Re-indexing dataset with embeddings to Solr
With a flag `FORCE_REINDEX`,  Solr reruns and re-indexes dataset with embeddings from 
`resources/embeddings/documents_embeddings.jsonl` without stopping Solr
```bash
